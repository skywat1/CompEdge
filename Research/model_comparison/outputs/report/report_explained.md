...
### A17. Prompt calibration pass (baseline → v1 → v2)

After the analysis above showed gemini-3.5-flash ranking at/above the human
ceiling but scoring systematically **high** (worst in kitchen and living room),
we edited the per-room scoring prompts to fix the *level* without disturbing the
*ranking*, and re-scored the same 152 images (gemini only, 1 call/image, ~$1.75
per pass). Two iterations:

- **v1** — strengthened "ignore staging," reset the "standard renovation =
  Level 4" baseline, and added per-room caps. Result: kitchen and living bias
  went to ~0, **but** the caps over-compressed the scale — the top of the range
  collapsed (share ≥5.5 fell to ~5% in every room), bathroom over-corrected to
  −0.61, and rank correlation dropped (living rho 0.81→0.68).
- **v2** — walked bathroom back to near-original, softened the numeric caps to
  "furniture/styling alone don't raise the score," and added an explicit "do not
  compress to the middle — premium permanent finishes still deserve 6–7" rule.

**Signed bias (model − human), all-3 reference — baseline → v1 → v2** (target 0):

| room        | baseline |    v1 |    v2 |
|:------------|---------:|------:|------:|
| kitchen     |    +0.60 | −0.05 | +0.20 |
| bathroom    |    +0.13 | −0.61 | −0.08 |
| bedroom     |    +0.47 | +0.27 | +0.34 |
| living_room |    +0.59 | +0.03 | +0.17 |
| **mean \|bias\|** | **0.45** | **0.24** | **0.20** |

**Spearman rho vs human, all-3 — baseline → v1 → v2** (higher = better; the last
column is the per-room human ceiling — the model beats it in every room at v2):

| room        | baseline |    v1 |    v2 | human ceiling |
|:------------|---------:|------:|------:|--------------:|
| kitchen     |    0.850 | 0.824 | 0.822 |         0.719 |
| bathroom    |    0.748 | 0.754 | 0.724 |         0.626 |
| bedroom     |    0.784 | 0.721 | 0.719 |         0.598 |
| living_room |    0.810 | 0.681 | 0.772 |         0.707 |
| **mean**    | **0.798** | **0.745** | **0.759** |           |

**Scale spread — share of images scored ≥5.5 and score std (baseline → v1 → v2).**
v1 collapsed the top of the scale; v2 restored it:

| room        | share ≥5.5 (b→v1→v2) | std (b→v1→v2)      |
|:------------|:---------------------|:-------------------|
| kitchen     | 32% → 5% → 16%       | 1.32 → 1.00 → 1.11 |
| bathroom    | 16% → 5% → 11%       | 1.08 → 0.90 → 1.01 |
| bedroom     | 16% → 5% → 11%       | 1.13 → 0.79 → 0.86 |
| living_room | 37% → 13% → 21%      | 1.29 → 1.05 → 1.07 |

**Verdict:** v2 is the chosen prompt. Vs baseline it cuts mean absolute bias
**55% (0.45 → 0.20)** with every room within ±0.35, restores the full scale
range, and keeps rank correlation **above the human ceiling in all four rooms**
(so the nominal rho dip vs baseline is not practically meaningful). Residual:
bedroom still runs ~+0.34 high, but bedroom has the lowest human ceiling and is
measured here on a mid-heavy 38-image sample with single (non-replicated) calls,
so further prompt tuning risks overfitting. Next steps are the anchor-grid images
(unchanged so far) and validation on a price-stratified sample that covers the
scale tails, before the full-corpus production run.
